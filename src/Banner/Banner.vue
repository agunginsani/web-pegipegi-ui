...
<script lang="ts" setup>
  type BannerProps = {
    color: string; // 'neutral' | 'accent' | 'info' | 'warning' | 'positive' | 'negative'
...
<template>
  <div
    class="flex gap-3 items-stretch rounded-lg max-w-xs py-2 pl-5 pr-11 relative"
    :class="wrapperStyle"
  >
    <div>
      <slot name="icon" />
    ...
    <button @click="onClose()">
      <svg
        ...
        :class="buttonStyle"
        class="absolute top-5 w-3.5 h-3.5"
      >
        <path
